...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        FC_relu1_out, FC_relu1_cache = affine_relu_forward(conv_relu_pool_out, W2, b2)
        FC2_out, FC2_cache = affine_forward(FC_relu1_out, W3, b3)
        ...
        for i in range(3):
            loss += 1/2 * self.reg * (np.sum(np.square(self.params['W'+str(i+1)])))

        dx3, dw3, db3 = affine_backward(dsoft, FC2_cache)
        dx2, dw2, db2 = affine_relu_backward(dx3, FC_relu1_cache)
